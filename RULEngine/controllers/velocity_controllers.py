...
class RealVelocityController(ControllerBaseClass):
    ...
    def execute(self, robot: Robot):
        ...
        if is_time_to_break(robot.pose, robot.path.points[-1], robot.cruise_speed, MAX_LINEAR_ACCELERATION):
            speed_norm = MIN_LINEAR_SPEED

        # TODO: test this IRL
        # speed_norm = optimal_speed(robot.pose, path.points[-1], robot.cruise_speed, MAX_LINEAR_ACCELERATION)
