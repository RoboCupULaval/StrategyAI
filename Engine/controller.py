...
class Controller:
    ...
    def execute(self) -> RobotState:
        ...
        for robot in self.active_robots:
            ...
            self.send_debug(robot, cmd)

            commands[robot.id] = self._put_in_robots_referential(robot, cmd)
    ...
    def send_debug(self, robot: Robot, cmd: Pose):
        ...
        self.ui_send_queue.put_nowait(DebugCommandFactory.plot_point('mm/s',
                                                                     'robot {} Kalman speed'.format(robot.id),
                                                                     [time.time()],
                                                                     [robot.velocity.norm]))
