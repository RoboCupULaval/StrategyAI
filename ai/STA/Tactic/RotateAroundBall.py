...
from RULEngine.Util.area import isFacingPointAndTarget, isInsideCircle
...
class RotateAroundBall(Tactic):
    ...
    def __init__(self, game_state, player_id, target):
        ...
        self.current_state = self.check_rotation_success
        self.next_state = self.check_rotation_success

    def check_rotation_success(self):
        ...
        if isFacingPointAndTarget(self.player_pos, self.origin, self.target.position, ANGLE_TO_HALT):
            ...
            self.next_state = self.calculate_next_rotation
    ...
    def calculate_next_rotation(self):
        ...
        self.new_pose = Pose(new_position, new_orientation)

        # return command
        self.next_state = self.apply_rotation
        go_to_position = MoveToPosition(self.game_state, self.player_id, self.new_pose)

        return go_to_position

    def apply_rotation(self):

        # check if done mini-rotating
        player_pose = self.game_state.get_player_pose(self.player_id)
        if (isInsideCircle(player_pose.position,self.new_pose.position,10)): #TODO: mettre dans tactic_const
            self.next_state = self.check_rotation_success
            return Idle(self.game_state, self.player_id)
        else:
            self.next_state = self.apply_rotation
