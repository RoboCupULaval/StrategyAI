...
class MakePass(Tactic):
    ...
    def __init__(self, game_state, player_id, receiver_position):
        Tactic.__init__(self, game_state, player_id)
        assert isinstance(player_id, int)
        assert PLAYER_PER_TEAM >= player_id >= 0
        assert isinstance(receiver_position, Position)
        # TODO : s'assurer de le receiver_position soit à l'intérieur du terrain
    ...
    def kick_ball_towards_target(self):
        if player_grabbed_ball(self.game_state, self.player_id, self.receiver_position):
            player_position = self.game_state.get_player_position(self.player_id)
            kick_force = get_required_kick_force(player_position, self.receiver_position)
