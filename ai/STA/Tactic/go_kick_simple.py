...
class GoKickSimple(Tactic):
    ...
    def go_behind_ball(self):
        ...
        required_orientation = (self.target.position - self.game_state.ball_position).angle
        position_behind_ball = self.get_destination_behind_ball(GO_BEHIND_SPACING)
        cruise_speed = 3
        ...
        return CmdBuilder().addMoveTo(Pose(position_behind_ball, required_orientation),
                                      ...
                                      end_speed=0,
                                      ball_collision=True).addChargeKicker().addKick(self.kick_force).build()
