...
class DefenseWall(Strategy):
    ...
    def _generate_cover_to_coveree_mapping(self):
        ...
        closest_enemies_to_ball = closest_players_to_point(self.game_state.ball_position, is_our_team=False)
        if len(closest_enemies_to_ball) > 0:
            closest_enemy_to_ball = closest_enemies_to_ball[0].player
            closest_enemies_to_our_goal = closest_players_to_point(self.game_state.field.our_goal, is_our_team=False)
