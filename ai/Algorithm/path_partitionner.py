...
class Path:
    ...
    @staticmethod
    def generate_path_from_points(points_list, speed_list=None):
    ...
    def quick_update_path(self, player):
        ...
        return self.generate_path_from_points(self.points, self.speeds)
...
class PathPartitionner(Pathfinder):
    ...
    def fastpathplanner(self, path, depth=0, avoid_dir=None, old_path=None, old_raw_path=None):
        ...
        if old_path is None:
            ...
            # print(self.is_path_collide(old_raw_path))
            if not self.is_path_collide(old_raw_path):
                if path.goal == old_path.goal:
    ...
    def get_path(self, player: OurPlayer, pose_target: Pose=Pose(), cruise_speed: [int, float]=1,
                 old_path: List=None, old_raw_path=None):
        ...
        self.path = self.fastpathplanner(self.path, old_path=old_path, old_raw_path=old_raw_path)
    ...
    def is_path_collide(self, path):
        for idx, points in enumerate(path.points[:-1]):
            dist = get_distance(path.points[idx+1], path.points[idx])
            if dist > 0.01:
                pose_start = path.points[idx].conv_2_np()
                direction = (path.points[idx+1].conv_2_np() -
                             pose_start) / np.linalg.norm(path.points[idx+1].conv_2_np() - pose_start)
    ...
    def search_point(self, path, avoid_dir=None):
        ...
        if 0 < len_along_path < get_distance(pose_target, pose_robot):
            vec_perp = np.cross(np.append(direction, 0), np.array([0, 0, 1]))  # TODO this
            ...
            if avoid_dir is None:
                ...
                # if abs(get_distance(path.start, Position(sub_target_1[0], sub_target_1[1])) -\
                #        get_distance(path.start, Position(sub_target_2[0], sub_target_2[1]))) < 300:
                #
                #     sub_target = sub_target_1
                #     avoid_dir = -vec_perp
                #
                # else:
                #     if get_distance(path.start, Position(sub_target_1[0], sub_target_1[1])) <\
                #        get_distance(path.start, Position(sub_target_2[0], sub_target_2[1])):
                #         sub_target = sub_target_1
                #         avoid_dir = -vec_perp
                #
                #     else:
    ...
    def reshape_path(self, path: Path, player: OurPlayer, vel_cruise: [int, float]=1000):
        ...
        for idx, point in enumerate(point_list[1:-1]):
            ...
            if np.linalg.norm(point_list[i] - point_list[i+1]) < 10:
                continue
            min_dist = 0.5 * (np.square(speed_list[i] - np.square(speed_list[i + 1])) / (OurPlayer.max_acc * 1000))
